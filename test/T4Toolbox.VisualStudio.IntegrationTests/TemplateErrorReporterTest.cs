...
namespace T4Toolbox.VisualStudio.IntegrationTests
{
    ...
    public class TemplateErrorReporterTest : IntegrationTest
    {
        ...
        [TestMethod]
        public async Task TemplateErrorReporterCreatesErrorItemsForTemplateErrorsAsync()
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
        ...
        [TestMethod]
        public async Task TemplateErrorReporterPopulatesLineAndColumnNumberInErrorItemsAsync()
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
            string tempFile = this.CreateTempFile(Path.GetTempPath(), "<#@");
            using (ITextDocument document = this.documentFactory.CreateAndLoadTextDocument(tempFile, this.textContentType))
        ...
        [TestMethod]
        public async Task TemplateErrorReporterCreatesErrorItemsThatNavigateToSourceOfErrorAsync()
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
        ...
        [TestMethod]
        public async Task TemplateErrorReporterClearsErrorItemsWhenTextDocumentIsDisposedAsync()
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
            string tempFile = this.CreateTempFile(Path.GetTempPath(), "<#@");
            using (ITextDocument document = this.documentFactory.CreateAndLoadTextDocument(tempFile, this.textContentType))
        ...
        [TestMethod]
        public async Task TemplateErrorReporterIsNotGarbageCollectedBeforeTextDocumentIsDisposedAsync()
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
        ...
        [TestMethod]
        public async Task TemplateErrorReporterCanBeGarbageCollectedAfterTextDocumentIsDisposedAsync()
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
        ...
        [TestMethod]
        public async Task TemplateErrorReporterUpdatesErrorItemsWhenTemplateChangesAsync()
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
        ...
        public async Task GetOrCreateStoresErrorProviderInBufferPropertiesAsync()
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
            string tempFile = this.CreateTempFile(Path.GetTempPath(), string.Empty);
            using (ITextDocument document = this.documentFactory.CreateAndLoadTextDocument(tempFile, this.textContentType))
        ...
        [TestMethod, SuppressMessage("Microsoft.Design", "CA1024:UsePropertiesWhereAppropriate", Justification = "GetOrCreate is the target method name")]
        public async Task GetOrCreateDoesNotCreateNewTemplateErrorReporterIfItAlreadyExistsForTextBufferAsync()
        {
        ...
        [TestMethod]
        public async Task DisposeRemovesErrorProviderFromBufferPropertiesAsync()
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
            string tempFile = this.CreateTempFile(Path.GetTempPath(), string.Empty);
            using (ITextDocument document = this.documentFactory.CreateAndLoadTextDocument(tempFile, this.textContentType))
